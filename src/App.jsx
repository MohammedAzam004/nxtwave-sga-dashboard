...
import ParentQueriesPage from "./ParentQueriesPage";
...
const ACTIVE_SESSION_SLOT = "9-10";
...
function createDefaultAttendanceSession(slot = ACTIVE_SESSION_SLOT) {
  return {
    slot,
    status: "pending",
    alertSent: false,
...
function getActiveSession(attendanceSessions = []) {
  return (
...
function updateActiveSession(attendanceSessions = [], updates) {
  const nextSessions = Array.isArray(attendanceSessions)
    ? [...attendanceSessions]
    : [];
  ...
  const nextSession = {
    ...
    alertSent:
      updates?.alertSent !== undefined
        ? Boolean(updates.alertSent)
        : Boolean(currentSession.alertSent),
  };
...
function readStoredRecordMap(storageKey) {
  ...
  if (
    !storedRecordMap ||
    typeof storedRecordMap !== "object" ||
    Array.isArray(storedRecordMap)
  ) {
    return {};
  }
...
function App() {
  ...
  const [studentQueriesById, setStudentQueriesById] = useState(() =>
    loadStoredQueriesByStudent(),
  );
  const [weeklyReportsByStudentId, setWeeklyReportsByStudentId] = useState(() =>
    loadStoredWeeklyReportsByStudent(),
  );
  const [currentPhase, setCurrentPhase] = useState(() =>
    normalizePhase(readStoredValue(ATTENDANCE_PHASE_STORAGE_KEY, "session")),
  );
  const [studentAttendanceAccessById, setStudentAttendanceAccessById] = useState(
    () => readStoredRecordMap(ATTENDANCE_ACCESS_STORAGE_KEY),
  );
  const [attendanceRequestsByStudentId, setAttendanceRequestsByStudentId] =
    useState(() => readStoredRecordMap(ATTENDANCE_REQUESTS_STORAGE_KEY));
    ...
    const loadStudents = async () => {
      ...
      try {
        ...
        if (!Array.isArray(studentData)) {
          throw new Error("Student records are not in the expected format.");
        }
  ...
  useEffect(() => {
    ...
    saveStoredValue(
      ...
      studentSessionState.attendanceHistoryById,
    );
    saveStoredValue(
      ATTENDANCE_ACCESS_STORAGE_KEY,
      studentAttendanceAccessById,
    );
    saveStoredValue(
      ATTENDANCE_REQUESTS_STORAGE_KEY,
  ...
  const studentsWithSessionData = students.map((student) => {
    ...
    const weeklyAttendanceSummary = calculateWeeklyAttendanceSummary(
      attendanceSessions,
      student.riskLevel,
    );
  ...
  const handleGenerateWeeklyReport = async (studentId) => {
    ...
    const currentStudent = students.find(
      (student) => String(student.id) === studentKey,
    );
    ...
    const attendanceSessions = normalizeAttendanceSessions(
      currentStudent.attendanceSessions,
    );
  ...
  const handleMarkAttendance = async (studentId, attendanceStatus) => {
    ...
    const attendedClasses =
      currentAttendedClasses + (normalizedStatus === "PRESENT" ? 1 : 0);
    const percentage = totalClasses
      ? Math.round((attendedClasses / totalClasses) * 100)
      : 0;
    ...
    const nextHistoryEntry = createAttendanceHistoryEntry(
      normalizedStatus,
      updatedStudent,
      createdAt,
      historyId,
    ...
    setStudentSessionState((currentState) => ({
      ...
      attendanceHistoryById: {
        ...
        [studentKey]: [
          nextHistoryEntry,
          ...(currentState.attendanceHistoryById[studentKey] || []),
        ],
      },
    }));
  ...
  const handleStartNewSession = () => {
    ...
    setStudentSessionState((currentState) => ({
      ...
      students: currentState.students.map((student) => ({
        ...student,
        attendanceSessions: updateActiveSession(
          normalizeAttendanceSessions(student.attendanceSessions),
          {
            status: "pending",
            alertSent: false,
            createdAt: new Date().toISOString(),
  ...
  const handleStudentMarkAttendance = async (studentId) => {
    ...
    const activeSession = getActiveSession(
      normalizeAttendanceSessions(currentStudent.attendanceSessions),
    );
    ...
    setStudentAttendanceAccessById((currentAccessByStudentId) =>
      updateAttendanceAccess(currentAccessByStudentId, studentId, {
        alreadyMarked: true,
      }),
    );
  ...
  const handleCloseSession = async () => {
    ...
    for (const student of pendingStudents) {
      ...
      const absenceAlert = await generateAbsenceAlert(
      ...
      setStudentSessionState((currentState) => ({
        ...
        students: currentState.students.map((currentStudent) => {
          if (String(currentStudent.id) !== String(student.id)) {
            return currentStudent;
          }
          ...
          return {
            ...
            alerts: [
              absenceAlert,
              ...ensureStudentAlertRecords(currentStudent.alerts),
            ],
          };
        }),
      }));
  ...
  const handleRequestAttendanceApproval = (studentId, reasonMessage) => {
    ...
    setAttendanceRequestsByStudentId((currentRequestsByStudentId) => {
      ...
      const hasPendingRequest = currentRequests.some(
        (request) => request.status === "pending",
      );

      if (hasPendingRequest) {
        return currentRequestsByStudentId;
      }

      return {
        ...currentRequestsByStudentId,
        [studentKey]: [
          createAttendanceRequest(studentId, trimmedReason),
          ...currentRequests,
        ],
      };
    });
